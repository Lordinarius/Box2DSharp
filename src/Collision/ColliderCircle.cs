...
namespace Box2DSharp.Collision
{
    public static partial class CollisionUtils
    {
            ...
            in Transform xfB)
        {
            ...
            {
                var s = V2.Dot(normals[i], cLocal - vertices[i]);
            ...
            if (separation < Settings.Epsilon)
            {
                ...
                manifold.LocalPoint = F.Half * (v1 + v2);
            ...
            var u1 = V2.Dot(cLocal - v1, v2 - v1);
            var u2 = V2.Dot(cLocal - v2, v1 - v2);
            if (u1 <= F.Zero)
            {
                if (V2.DistanceSquared(cLocal, v1) > radius * radius)
                {
                    return;
                }
            ...
            {
                if (V2.DistanceSquared(cLocal, v2) > radius * radius)
                {
                    return;
                }
            ...
            {
                var faceCenter = F.Half * (v1 + v2);
                var s = V2.Dot(cLocal - faceCenter, normals[vertIndex1]);
                if (s > radius)
                {
                    return;
                }
