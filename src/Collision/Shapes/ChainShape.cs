...
namespace Box2DSharp.Collision.Shapes
{
    ...
    public class ChainShape : Shape
    {
        ...
        public V2 PrevVertex;

        public V2 NextVertex;
        ...
        {
            var clone = new ChainShape {Vertices = new V2[Vertices.Length]};
        ...
        public void CreateLoop(V2[] vertices, int count = -1)
        {
            if (count == -1)
            {
            ...
            for (var i = 1; i < count; ++i)
            {
                ...
                Debug.Assert(V2.DistanceSquared(v1, v2) > Settings.LinearSlop * Settings.LinearSlop);
            }

            Count = count + 1;
            Vertices = new V2[Count];
        ...
        public void CreateChain(V2[] vertices, int count, V2 prevVertex, V2 nextVertex)
        {
            Debug.Assert(Vertices == null && Count == 0);
            ...
            for (var i = 1; i < count; ++i)
            {
                // If the code crashes here, it means your vertices are too close together.
                Debug.Assert(
                    V2.DistanceSquared(vertices[i - 1], vertices[i])
        ...
        public override void ComputeAABB(out AABB aabb, in Transform transform, int childIndex)
        {
            ...
            var lower = V2.Min(v1, v2);
            var upper = V2.Max(v1, v2);
        ...
        public override void ComputeMass(out MassData massData, F density)
        {
            ...
            massData.Mass = F.Zero;
            massData.Center.SetZero();
            massData.RotationInertia = F.Zero;
        }
    }
}